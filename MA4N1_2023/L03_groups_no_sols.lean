...
--  Did you use `aux` in the previous example?
--  If so, find a proof that does not use it.
--  If not, then use it now!
example : a⁻¹ * (b * c) * a = (a⁻¹ * b * a) * (a⁻¹ * c * a) := by
  sorry
  done
